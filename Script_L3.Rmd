...
```{r cov and corr, message = FALSE, warning = FALSE}
# Covariance matrix
cov_mat <- cov(anxiety)
cov_mat
...
Keep in mind that loops can very helpful to perform repetitive processes, and they are at the core of any programming languages. In the case of *R*, there are also some additional function that perform some sort of loops for some particular situations. For those who are more interested on this, you can check the function *apply* and its variations.
...
As we see, according to the screeplot and the parallel analysis, we should keep three factors. How would you interpret these factors? What can wee see in here? 

Furthermore, we can inspect some of the fit measures to evaluate if the model that we are selecting is the most adequate for our data.
...
cor_mat
```
Finally, we can retrieve our latent variables and store them in the data set
